...
$RegSimResult = mysql_query("SELECT * FROM `gm_madden08-ATL_3_info` where Field='SimReg'");
...
if ($RegSimrow['Value'] != '') {
    ...
    $result = mysql_query("SELECT * FROM `gm_madden08-ATL_3_reg`");
    ...
    if (!isset($_SESSION['Admin'])) {
        ...
        while ($row = mysql_fetch_array($result)) {
            echo '<tr>';
            echo '<td>', $row['Week'], '</td>', '<td id="', $row['Row'], '/ATL/3/Vs" onclick="updateReg(this)">', $row['Vs'], '</td>', '<td id="', $row['Row'], '/ATL/3/HorA" onclick="updateReg(this)">', $row['HorA'], '</td>', '<td id="', $row['Row'], '/ATL/3/Score" onclick="updateReg(this)">', $row['Score'], '</td>', '<td id="', $row['Row'], '/ATL/3/Result" onclick="updateReg(this)">', $row['Result'], '</td>', '<td id="', $row['Row'], '/ATL/3/OvrRecord" onclick="updateReg(this)">', $row['OvrRecord'], '</td>', '<td id="', $row['Row'], '/ATL/3/DivRecord" onclick="updateReg(this)">', $row['DivRecord'], '</td>';
            echo '</tr>';
        }
    }
