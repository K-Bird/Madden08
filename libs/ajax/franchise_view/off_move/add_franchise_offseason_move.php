...
if ($moveType === 'retired') {
    ...
    $addNewMove = db_query("Insert into `franchise_year_off_moves` (Player, Position, Overall, Age, Year, Type, Team, Master_Row_ID) Values ('{$playerName}','{$position}','{$ovr}','{$age}','{$franYear}','{$moveType}', '{$franchise}', '{$master_row}')");
    $getMovesMax = db_query("SELECT MAX(`Row`) as MaxMove FROM `franchise_year_off_moves`");
    $getMoveRow = $getMovesMax->fetch_assoc();
    $MoveMax = $getMoveRow['MaxMove'];
    $addFAToStaging = db_query("INSERT INTO `franchise_staging_freeagency` (Name, Position, Overall, Age, Rookie, Franchise, Year, MoveRow, Master_Row_ID) Values ('{$playerName}','{$position}','{$ovr}','{$age}','','{$franchise}','{$franYear}','{$MoveMax}','{$master_row}')");
} else {
    $addNewMove = db_query("Insert into `franchise_year_off_moves` (Player, Position, Overall, Age, Draft, Year, Type, Team) Values ('{$playerName}','{$position}','{$ovr}','{$age}','{$draft}','{$franYear}','{$moveType}', '{$franchise}', '{$master_row}')");
}
